...
class Latency_view(tk.Frame):
    ...
    def calculate_latency_and_packet_loss(self, sender_pcap_path, receiver_pcap_path):
        ...
        print(f"Sender cap path: {sender_pcap_path}")
        sender_cap = pyshark.FileCapture(sender_pcap_path_filtered)
        ...
        print(f"Receiver cap path: {receiver_pcap_path}")
        receiver_cap = pyshark.FileCapture(receiver_pcap_path_filtered)
    ...
    def calculate(self):
        try:
            print("Calculating...")
            average_latency, lost_packets = self.calculate_latency_and_packet_loss('pcaps/ue_uplink_2Mbps_256bytes_1.pcap', 'pcaps/core_uplink_2Mbps_256bytes_1.pcap')
...
'''def calculate(self):
        pcaps_dir = 'pcaps'
        pcap_files = [f for f in os.listdir(pcaps_dir) if f.endswith('.pcap')]
        paired_files = {}

        # Paso 1: Agrupar los archivos por su nombre base, ignorando el prefijo "ue_" o "core_"
        for file in pcap_files:
            base_name = re.sub(r'^(ue_|core_)', '', file)
            if base_name not in paired_files:
                paired_files[base_name] = []
            paired_files[base_name].append(file)

        # Paso 2: Iterar sobre los grupos de archivos emparejados
        for base_name, files in paired_files.items():
            if len(files) == 2:  # Asegurarse de que hay una pareja
                # Determinar cuál es el sender y cuál el receiver basado en "uplink" o "downlink"
                if 'uplink' in base_name:
                    sender = [f for f in files if f.startswith('ue_')][0]
                    receiver = [f for f in files if f.startswith('core_')][0]
                elif 'downlink' in base_name:
                    sender = [f for f in files if f.startswith('core_')][0]
                    receiver = [f for f in files if f.startswith('ue_')][0]
                else:
                    continue  # Si no es ni uplink ni downlink, continuar con el siguiente grupo

                sender_pcap_path = os.path.join(pcaps_dir, sender)
                receiver_pcap_path = os.path.join(pcaps_dir, receiver)

                try:
                    print("Calculating...")
                    average_latency, lost_packets = self.calculate_latency_and_packet_loss(sender_pcap_path, receiver_pcap_path)
                    print("Calculations done!")
                    if average_latency == 0 or lost_packets == 0:
                        print("The metrics couldn't be calculated. Please try again.")
                    else:
